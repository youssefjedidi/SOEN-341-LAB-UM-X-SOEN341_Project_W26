...
export default function WeeklyPlanner() {
  ...
  // Derives total weekly calories from daily planner data
  const weeklyTotalCalories = useMemo(() => {
    return days.reduce((sum, day) => {
      return sum + (dayCalories[day] ?? 0);
    }, 0);
  }, [dayCalories]);
  ...
  //Calculates progress towards weekly goal as a percentage
  const progressPercentage = isGoalValid
    ? Math.min((weeklyTotalCalories / weeklyGoal) * 100, 100)
    : 0;

  //Derives status text based on how the weekly total compares to the goal
  const statusText = !isGoalValid
    ? "No max set"
        ...
        <div className={layoutStyles.sectionSpacing}>
          <div className={layoutStyles.responsiveTableWrapper}>
            <div className={layoutStyles.plannerGrid}>
              ...
              {/* Weekly average row */}
              <div className={layoutStyles.weeklyRowLabel}>
                <span className="text-xs md:text-sm font-black uppercase tracking-widest text-stone-800">
                  Weekly Avg
                </span>
              </div>

              <div className={layoutStyles.weeklyCell}>
                <div className="flex items-center justify-between gap-2">
                  <span className={formStyles.statText}>
                    {weeklyTotalCalories} / {weeklyGoal ?? "—"} cal
                  </span>
                </div>

                <div className={layoutStyles.progressBarContainer}>
                  <div
                    className={`h-full transition-all duration-300 ${isGoalValid
                      ? getBarColorClass(averageCalories)
                      : "bg-stone-400"
                      }`}
                ...
                <p className={formStyles.helperStatText}>
                  {statusText}
                </p>
              </div>
            </div>
                ...
                {availableRecipes.map((recipe) => {
                  ...
                  return (
                    <button
                      ...
                      className={`${formStyles.cardListItem} ${isUsed
