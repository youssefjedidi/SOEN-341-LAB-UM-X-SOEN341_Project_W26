...
// Read migration file
const migrationPath = path.join(__dirname, '../supabase/migrations/20260205232142_create_user_profiles_table.sql');
const migrationSQL = fs.readFileSync(migrationPath, 'utf8');

async function setupDatabase() {
  try {
    console.log(`${colors.blue}Step 1:${colors.reset} Creating SQL executor function...`);
    ...
    const { error: funcError } = await supabase.rpc('execute_sql', { sql_text: createFunctionSQL });
    
    if (funcError && funcError.message.includes('function execute_sql(sql_text text) does not exist')) {
      // Function doesn't exist, we need to create it via a different method
      console.log(`${colors.yellow}   Creating executor function for the first time...${colors.reset}`);
      ...
      if (!response.ok) {
        throw new Error('Could not create executor function. Please run the initial setup SQL manually.');
      }
    }
    ...
    const { data, error } = await supabase.rpc('execute_sql', { sql_text: migrationSQL });
    ...
    console.log(`${colors.green}✅ Success! Database is ready!${colors.reset}\n`);
    console.log(`${colors.cyan}Verify:${colors.reset} Check Supabase Dashboard → Table Editor\n`);
    
  } catch (error) {
    console.error(`\n${colors.red}❌ Setup failed: ${error.message}${colors.reset}\n`);
    
    console.log(`${colors.yellow}📋 Manual Setup Required (One-Time):${colors.reset}\n`);
    console.log(`Run this SQL in Supabase SQL Editor:\n`);
    console.log(`${colors.cyan}CREATE OR REPLACE FUNCTION execute_sql(sql_text TEXT)`);
    ...
    console.log(`Then run: ${colors.cyan}npm run setup-db${colors.reset} again\n`);
